...
def get_repositories():
    ...
    response = requests.get(repos_endpoint, auth=(USERNAME, PASSWORD))

    if response.status_code == 200:
        all_repos = response.json()
        filtered_repos = [
            repo['key'] for repo in all_repos if repo['key'].endswith('-docker-snapshots')]
...
def cleanup_repository(repo_name, created, dry_run):
    ...
    if response.status_code == 200:
        ...
        if not grouped_items:
            logger.info(f"Nothing to clean up in repository: {repo_name}")
            return
...
def main(created, dry_run):
    repositories = get_repositories()
    for repo_name in repositories:
        logger.info(f"Cleaning up repository: {repo_name}")
        cleanup_repository(repo_name, created, dry_run)
...
if __name__ == '__main__':
    ...
    main(args.created, args.dry_run)
